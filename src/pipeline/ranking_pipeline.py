...
from src.config import CANDIDATE_LOOKUP_PATH, DEFAULT_RETRIEVAL_TOP_K
...
class RankingPipeline:
    ...
    def __init__(self):
        ...
        if not isinstance(self.candidate_lookup, dict) or not self.candidate_lookup:
            raise ValueError(
                f"The candidate lookup cache at '{CANDIDATE_LOOKUP_PATH}' is empty or invalid."
            )
    ...
    def rank(
        ...
        """Rank candidates for a job description without changing scoring logic."""
        if top_k <= 0:
        ...
        retrieval_top_k = max(DEFAULT_RETRIEVAL_TOP_K, top_k)
        semantic_results = self.retriever.search(jd_text, top_k=retrieval_top_k)
        ...
        for item in semantic_results:
            ...
            candidate = self.candidate_lookup.get(candidate_id)
            if candidate is None:
                raise ValueError(
                    "Candidate lookup cache does not contain candidate_id "
                    f"'{candidate_id}' referenced by the FAISS index."
                )
            ...
            skills = SkillRanker.score(candidate["skills"], jd["required_skills"])
            behavior = BehaviorRanker.score(candidate["behavioral"])
            career = CareerRanker.score(candidate["career_history"])
            recruiter = self.recruiter.score(candidate)

            honeypot_penalty = self.honeypot_detector.score(candidate)
        ...
        reranking_time = perf_counter() - reranking_start
        total_execution_time = perf_counter() - total_start

        self.logger.info("Retrieval time: %.3fs", retrieval_time)
        self.logger.info("Reranking time: %.3fs", reranking_time)
